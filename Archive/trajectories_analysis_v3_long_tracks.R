...
for (condition in condition_folders) {
  ...
  for (x in filenames){  
    ...
    for(i in 1:length(df_list_long_traj)){
      if (nrow(df_list_long_traj[[i]])-1 > 200) {
    ...
    if (dim(df_for_plot)[1] != 0) {
      ...
      #   theme_minimal() +
      #   ggtitle(paste(condition, "trajectories, mobile fraction only, filtered", sep=": ")) +
      #   xlab("Displacement smoothed by window = 5") + 
      #   ylab("Time") +
      ...
      #   scale_y_reverse(breaks = seq(0, 300, by = 50)) +
      #   #scale_y_continuous() +
      #   scale_x_continuous(limits = c(-2, 4), breaks = seq(-2, 4, by = 2))
      # #example output of filtered trajectories
      # svg(paste(paste("../trajectories_example_", x, sep=""), ".svg", sep=""),  width=3, height=3)
      # print(p)
      # dev.off()
    }  
    #####
    ...
    state_changes_df <- create_empty_table(10,1)
    ...
    if(length(changes_datalist) != 0) {
      ...
      assembled_state_changes <- setNames(assembled_state_changes,c("RS","AS","RA","AR","SR","SA","all_to_stop","stop_to_all","retr_to_stop_ant","ant_to_stop_retr"))
