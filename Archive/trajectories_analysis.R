...
for (condition in condition_folders) {
  ...
  for (x in filenames){  
    ...
    p <- ggplot(df_subset_naomit, aes(x = smoothed_x_displacement, y = Time)) + 
      ...
      geom_point(aes(color=motion), size=0.7) +
      theme_bw() +
    ...
    pdf(paste(paste("../trajectories_example_", x, sep=""), ".pdf", sep=""),  width=5.5, height=3.5)
    print(p)
    dev.off()
