...
condition_folders <- Sys.glob(file.path("*APP*")) #check that only required folders start with Tau*
...
for (condition in condition_folders) {
  print(condition)
  #condition <- "20210704_PC12_APP_DMSO_0.01"
  setwd(paste("C:/Users/NITru/Downloads/Axonal_Transport/",condition,sep=""))
  
  ###REMOVE FILES FROM PREVIOUS
  f1 <-  Sys.glob(file.path("*df_for_plot.csv"))
  file.remove(f1) # remove the files
  f2 <-  Sys.glob(file.path("*fractions*"))
  file.remove(f2)
  f3 <-  Sys.glob(file.path("*changes*"))
  file.remove(f3)
  f4 <-  Sys.glob(file.path("*velocities*"))
  file.remove(f4)
  ...
  full_ID_table <- read_excel("../TibcoSpotfire_export_together.xlsx", sheet = "Sheet1")
  ...
  nlevels(mobile_ID_table$`Original ID`)
  ...
  for (x in filenames){  
    #print(x)
    #x <-"20210328_PC12_APP_0.01%_DMSO_cell001_czi_606c047032527_hrm.xlsx"
    df <- read_excel(x, sheet = "Position Reference Frame", skip = 1) # no reference frame displacement tab
    df$TrackID <- as.factor(df$TrackID)
    df_subset <- df[df$TrackID %in% mobile_ID_table$`Original ID`,]
    
    #df_subset <- df_subset[which(df_subset$TrackID == "1000000119"),] #to plot one trajectory 1000037660 Tau421_001percentDMSO, Tau421_APP-GFP_0-01percentDMSO_cell16_czi_5ecbdd0f6adf5_hrm.xlsx
    df_subset <- droplevels(df_subset)
    if (nlevels(df_subset$TrackID) != 0) {
      print(nlevels(df_subset$TrackID))
      ...
      # ggplot(df_subset, aes(x = `Position X Reference Frame`, y = `Position Y Reference Frame`)) +
      ...
      ggplot(df_subset, aes(x = `Position X Reference Frame`, y = `Position Y Reference Frame`)) +
        ...
        theme(legend.position = "none") +
        #theme_classic() +
        xlab("Position X Reference Frame") +
        ylab("Position Y Reference Frame") +
      ...
      df_list_0 <- list()
      df_list_0 <- split(df_subset, df_subset$TrackID)
      #To avoid uninitialized column warning try:
      df_subset$x_displacement <- NA
      df_subset$y_displacement <- NA
      if (length(df_list_0) >= 1) {
        for(i in 1:(length(df_list_0))){
          #print(df_list[[i]]$TrackID[1])
          if (nrow(df_list_0[[i]]) > 1) {
            for (j in 1:nrow(df_list_0[[i]])) {
              #print(df_list_0[[i]]$`Position X Reference Frame`[[j]]-df_list_0[[i]]$`Position X Reference Frame`[[1]])
              df_list_0[[i]]$x_displacement[[j]] <- as.numeric(df_list_0[[i]]$`Position X Reference Frame`[[j]]-df_list_0[[i]]$`Position X Reference Frame`[[1]])
              df_list_0[[i]]$y_displacement[[j]] <- as.numeric(df_list_0[[i]]$`Position Y Reference Frame`[[j]]-df_list_0[[i]]$`Position Y Reference Frame`[[1]])
            }
          }  
        }
      }
      
      df_subset <- do.call(rbind, df_list_0)
      
      #To avoid uninitialized column warning try:
      df_subset$num <- NA
      df_subset$smoothed_x_displacement <- NA
      
      df_subset$num <- ave(df_subset$x_displacement, df_subset$TrackID, FUN = seq_along)
      ...
      df_subset$smoothed_x_displacement <- df_subset$x_displacement
      ...
      ggplot(df_subset, aes(x = `Position X Reference Frame`, y = `Position Y Reference Frame`)) +
        ...
        #dark_theme_gray() +
        theme(legend.position = "none") +
        geom_path(aes(x = `Position X Reference Frame`, y = `Position Y Reference Frame`, color = TrackID), linetype = "dashed") +
        #theme_classic() +
        xlab("Position X Reference Frame") +
        ylab("Position Y Reference Frame") +
        #scale_y_reverse()  +
      ...
      ggplot(df_subset, aes(x = `Position X Reference Frame`, y = Time)) +
        geom_path(aes(color = TrackID)) +
        scale_color_manual(values = cbp) +
        #dark_theme_gray() +
        theme(legend.position = "none") +
        geom_path(aes(x = `Position X Reference Frame`, y = Time, color = TrackID), linetype = "dashed") +
        #theme_classic() +
      ...
      df_subset$x_displacement <- unlist(df_subset$x_displacement)
      ggplot(df_subset, aes(x = x_displacement, y = Time)) +
        geom_path(aes(color = TrackID)) +
        scale_color_manual(values = cbp) +
        #dark_theme_gray() +
        theme(legend.position = "none") +
        geom_path(aes(x = x_displacement, y = Time, color = TrackID), linetype = "dashed") +
        #theme_classic() +
        ggtitle("Trajectories, mobile fraction only, cut values out smoothing range") +
        xlab("Displacement X Reference Frame") +
        ylab("Time") +
    ...
    #df_for_plot <- as.matrix(df_for_plot)
    #write.table(df_for_plot, paste(x, "_df_for_plot.csv", sep=""), sep = ";",dec = '.', row.names = FALSE, col.names = TRUE)
